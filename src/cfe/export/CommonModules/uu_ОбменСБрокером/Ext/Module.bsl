...
Процедура ВыполнитьОтправкуСообщений() Экспорт 
	...
	Пока Истина Цикл
		...
		Пока ВыборкаСообщения.Следующий() Цикл	
			...
			ДанныеНаЗапись.Добавить(Новый Структура("Key,Value,Headers", XMLСтрока(ВыборкаСообщения.Объект), ВыборкаСообщения.ТекстСообщения, Заголовки));			
			КлючиСообщений.Добавить(Новый Структура("Топик,Объект,ДатаДобавления", ВыборкаСообщения.Топик, ВыборкаСообщения.Объект, ВыборкаСообщения.ДатаДобавления));
		КонецЦикла; 
		...
		ЕстьСообщения = ВыборкаСообщения.Количество() > 0;
...
Процедура ОбработатьОтправляемыеСообщения(МассивСообщений, ОшибкаОтправки = Неопределено, ОтчетОДоставке = Неопределено)
	...
	Для Каждого КлючСообщения Из МассивСообщений Цикл
		...
		Ключ = XMLСтрока(КлючСообщения.Объект);
